...
class onEventController {
  ...
  openChatWithLink = async (match = null) => {
    const { init, Alert } = this.rootprops
  ...
  openChatWithFilter = async match => {
    const { init, Alert } = this.rootprops
  ...
  autoLogin = async (idToken = null) => {
    const { Alert, appCore, chatServiceGet, cookies, rtkQuery } = this.rootprops
    const { createUUID } = appCore.appcore
    const { history, setUser, currentCategory } = this.c.props
    let uniqueId = 'mac_' + createUUID()
  ...
  authOpen = async () => {
    const { chatServiceGet, dispatch, appApi, rtkQuery, cookies } = this.rootprops
