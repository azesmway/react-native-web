...
import HomeIcon from '../images/home-pngrepo-com.png'
...
import StarIcon from '../images/trophy-pngrepo-com.png'
...
const ViewMapGeo = ({ filter, currentCategory, user, places, history, selectHotel, selectPlace, setHeaderParams, setFooterBar, utils }) => {
  const { chatServiceGet, MapView, Marker, Spinner, Modal, Portal, Icon, theme, isMobile, Geolocation, MarkerClusterer } = utils
  ...
  useEffect(() => {
    ...
    Geolocation.getCurrentPosition(async info => {
      if (info.coords.latitude > 0 && info.coords.longitude > 0) {
        ...
        setGeo([55.755864, 37.617698])
      }
    })
  }, [])

  const getRequestRegion = async rg => {
    return await chatServiceGet.getRegionByGeo(rg.zoom, rg.minLat + ',' + rg.minLng + ',' + rg.maxLat + ',' + rg.maxLng)
  }
  ...
  // useEffect(() => {
  //   if (region.minLng !== 0 && region.zoom >= 6) {
  //     setSpinner(true)
  ...
  useEffect(() => {
    if (!spinner && currentRegion) {
      mapRef?.current?.getCamera().then(coords => {
        ...
        getRequestRegion(reg).then(result => {
          const m = result.filter(r => r.oid !== 0 && selectHotel && r.oid !== selectHotel.id)
          setMarkers(m)
        })
      })
    }
  }, [spinner, currentRegion])
  ...
  const MyClusterComponent = props => {
    return (
      <Marker onPress={props.onPress} coordinate={props.coordinate} anchor={{ x: 0.5, y: 0.5 }}>
        <View
          style={{
          ...
          <Text style={{ fontWeight: '700', fontSize: 20, color: '#fff' }}>{props.pointCountAbbreviated}</Text>
        </View>
      </Marker>
    )
  }
  ...
  const ViewHotel = ({ hotel }) => {
    ...
    const img = hotel.cover_image ? { uri: hotel.cover_image } : placeholder
      ...
      <>
        ...
        <View style={{ justifyContent: 'space-between', flexDirection: 'row', marginTop: 30 }}>
          <TouchableOpacity
            ...
            <Image source={ChatIcon} style={{ width: SCALE, height: SCALE, opacity: 0.5 }} />
          ...
          <TouchableOpacity
            ...
            style={{ alignItems: 'center', flex: 0.33 }}>
            <Image source={StarIcon} style={{ width: SCALE, height: SCALE, opacity: 0.5 }} />
          ...
          <TouchableOpacity
            ...
            style={{ alignItems: 'center', flex: 0.33 }}>
            <Image source={HomeIcon} style={{ width: SCALE, height: SCALE, opacity: 0.5 }} />
  ...
  const latitude = places && places[0] && places[0].lat ? Number(places[0].lat) : geo.length > 0 ? geo[0] : 55.755864
  const longitude = places && places[0] && places[0].lon ? Number(places[0].lon) : geo.length > 0 ? geo[1] : 37.617698
  ...
  return (
    <>
      ...
      <MapView
        ...
        onRegionChangeComplete={curRegion => {
          setCurrentRegion(curRegion)
        }}
      ...
      {modalView && (
        <Portal>
          <Modal
            ...
            <View style={{ width: isMobile ? width : width / 3, height: 300, backgroundColor: '#fff', borderRadius: 10, borderWidth: 0.5, padding: 10 }}>
              {loadingHotel ? (
